...
class CrudController{
    ...
    //    ozgaruvhilarni elon qilish
       foreach ($this->tableKey as $key => $value) {
        ...
        foreach ($this->tableKey as $key => $value) {
            if(count($this->tableKey)-1===$key){
                $this->keySqlValue = $this->keySqlValue."'".'$this->'.$value."'"; 
            }else{
                $this->keySqlValue = $this->keySqlValue."'".'$this->'.$value."',"; 
            }
        }
      ...
      foreach ($this->tableKey as $key => $value) {
            if(count($this->tableKey)-1===$key){
                $this->sqlUpdate = $this->sqlUpdate."$value="."'".'$this->'.$value."'"; 
            }else{
                $this->sqlUpdate = $this->sqlUpdate."$value="."'".'$this->'.$value."',"; 
            }
        }
    ...
    class $this->ControllerName  
     {
        ...
        public function deleteKey(".'$key,$id'."){
            try{
                ".'$sql="DELETE FROM `'.$this->tableName.'` WHERE $key='.'$id'.'";'."  
                if(".'$this->db'."->query(".'$sql'.")){
                    http_response_code(200);
                    return array('xabar'=>'delete users');
                }else {
        ...
        public function required(".'$required'.")
        {
            try {
                ...
                foreach ($key__ as $key => $value) {
                    $res = [...$this->db->query("SELECT * FROM `'.$this->tableName.'` WHERE $value='."'".'$required[$value]'."'".'")];'."
                    if (count(".'$res) > 0) {

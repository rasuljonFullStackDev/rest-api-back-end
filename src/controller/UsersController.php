    ...
    class UsersController  
     {
        ...
        public function create(){
            try{
                $sql="INSERT INTO `users`  ( `username` , `password` , `email` , `img` ) VALUES ('$this->username','$this->password','$this->email','$this->img')";
        ...
        public function update($id){
            try{
                $sql="UPDATE `users`  SET username='$this->username',password='$this->password',email='$this->email',img='$this->img' WHERE id='$id'";  
                if($this->db->query($sql)){
        ...
        public function deleteKey($key,$id){
            try{
                $sql="DELETE FROM `users` WHERE $key=$id";  
                if($this->db->query($sql)){
                    http_response_code(200);
                    return array('xabar'=>'delete users');
                }else {
        ...
        public function required($required)
        {
            try {
                ...
                foreach ($key__ as $key => $value) {
                    $res = [...$this->db->query("SELECT * FROM `users` WHERE $value='$required[$value]'")];
                    if (count($res) > 0) {
